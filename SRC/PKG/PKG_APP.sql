...
CREATE OR REPLACE PACKAGE SISTER_TST.PKG_APP AS
  ...
  -- Lettura di una chiave da un Application Context.
  -- pChiave:     nome dell'attributo da leggere (case insensitive in Oracle).
  -- pNamespace:  contesto da cui leggere (default: CTX_APP_PAR).
  --              Passare esplicitamente il namespace per accedere a contesti diversi,
  --              ad esempio CTX_APP_IDS, CTX_APP_ABL, CTX_APP_FLT.
  -- Restituisce NULL se la chiave non esiste nel namespace indicato.
  -- Esempio: GetParametro('DEBUG_LEVEL')           → valore da CTX_APP_PAR
  --          GetParametro('ID_UTENTE', 'CTX_APP_IDS') → valore da CTX_APP_IDS
  FUNCTION  GetParametro(pChiave IN VARCHAR2, pNamespace IN VARCHAR2 DEFAULT 'CTX_APP_PAR') RETURN VARCHAR2;

  -- Scrittura di una chiave in un Application Context.
  -- pChiave:    nome dell'attributo da impostare.
  -- pValore:    valore da assegnare (massimo 4000 caratteri per Oracle SYS_CONTEXT).
  --             Passare NULL per cancellare l'attributo dal contesto.
  -- pNamespace: contesto di destinazione (default: CTX_APP_PAR).
  --             Passare esplicitamente il namespace per scrivere in contesti diversi.
  --             Il package deve essere autorizzato a scrivere nel namespace indicato
  --             (il contesto deve essere stato creato con USING PKG_APP).
  -- Esempio: SetParametro('DEBUG_LEVEL', '3')
  --          SetParametro('ID_SESSIONE', vId, 'CTX_APP_IDS')
  PROCEDURE SetParametro(pChiave IN VARCHAR2, pValore IN VARCHAR2, pNamespace IN VARCHAR2 DEFAULT 'CTX_APP_PAR');
...
CREATE OR REPLACE PACKAGE BODY SISTER_TST.PKG_APP AS
  ...
  -- Lettura di un parametro da un Application Context tramite SYS_CONTEXT.
  -- Se pNamespace non viene specificato, legge da CTX_APP_PAR.
  -- Restituisce NULL se la chiave non esiste o se il contesto non è inizializzato.
  -- Nota: SYS_CONTEXT è thread-safe e non richiede locking.
  FUNCTION GetParametro(pChiave IN VARCHAR2, pNamespace IN VARCHAR2 DEFAULT 'CTX_APP_PAR') RETURN VARCHAR2 IS
	  BEGIN
	    RETURN SYS_CONTEXT(pNamespace, pChiave);
	  END GetParametro;


  -- Scrittura di un parametro in un Application Context tramite DBMS_SESSION.SET_CONTEXT.
  -- Se pNamespace non viene specificato, scrive in CTX_APP_PAR.
  -- Passare pValore = NULL per rimuovere l'attributo dal contesto (equivale a CLEAR_CONTEXT per la singola chiave).
  -- Attenzione: il contesto indicato in pNamespace deve essere stato creato con USING PKG_APP,
  --             altrimenti Oracle solleverà ORA-01031 (privilegi insufficienti).
  PROCEDURE SetParametro(pChiave IN VARCHAR2, pValore IN VARCHAR2, pNamespace IN VARCHAR2 DEFAULT 'CTX_APP_PAR') IS
	  BEGIN
	    DBMS_SESSION.SET_CONTEXT(namespace => pNamespace, attribute => pChiave, value => pValore);
	  END SetParametro;
  ...
  FUNCTION GetContatorLog RETURN NUMBER IS
	  BEGIN
	    RETURN NVL(TO_NUMBER(SYS_CONTEXT('CTX_APP_PAR', 'LOG_CONTATORE')), 0);
	  END GetContatorLog;
  ...
  FUNCTION VerificaAccesso(pTipoAzione IN VARCHAR2, pOggetto IN VARCHAR2, pAmbito IN VARCHAR2, pControlliLogici IN BOOLEAN DEFAULT TRUE) RETURN OBJ_Esito IS
    vIdAzione     NUMBER;
